...
public interface FriendReqRepo extends CrudRepository<FriendRequest,Serializable> {
	...
	List<FriendRequest> findfriendReq(String name);
	 ...
	 @Query(value = "select * from friend_request where friend_req_to=?1 and accepted='0'", nativeQuery = true)
	 List<FriendRequest> getpendingFriendReq( String friend_req_to );
	 
	 @Query(value = "select * from friend_request where friend_req_to=?1 and accepted='1'", nativeQuery = true)
	 List<FriendRequest> getFriendList( String friend_req_to );

	 @Query(value = "select * from friend_request where accepted='1'", nativeQuery = true)
